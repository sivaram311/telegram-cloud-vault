...
client = TelegramClient(
    "user_personal_session", 
    user_engine.API_ID, 
    user_engine.API_HASH,
...
async def backup_all_chats():
    ...
    while True:
        try:
            ...
            user_engine.logger.error(f"Client start failed ({e}). Retrying in 10s...")
    ...
    for idx, dialog in enumerate(dialogs, 1):
        ...
        while True:
            try:
                ...
                break  # Dialog iteration completed successfully

            except (ConnectionError, errors.RPCError, asyncio.TimeoutError) as net_err:
                user_engine.logger.warning(f"Network glitch on chat {chat_title} ({net_err}). Reconnecting in 15s...")
                await asyncio.sleep(15)
                checkpoint = latest_msg_id  # resume from last seen message
